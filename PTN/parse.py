...
class PTN:
    ...
    def process_title(self) -> None:
        ...
        if unmatched:
            ...
            # Corrige bug: ignora valores None em self.part_slices.values()
            part_slices_values = [s for s in self.part_slices.values() if s is not None]
            if len(self.part_slices) > 3 and len(part_slices_values) > 3 and title_start > sorted(part_slices_values, key=lambda s: s[0])[3][0]:
